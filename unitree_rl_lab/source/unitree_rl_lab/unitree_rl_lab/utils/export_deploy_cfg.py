...
def export_deploy_cfg(env: ManagerBasedRLEnv, log_dir):
    ...
    for obs_name, obs_cfg in obs_terms:
        obs_func = obs_cfg.func
        if not callable(obs_func):
            raise TypeError(f"Observation term '{obs_name}' has non-callable func: {type(obs_func)}")
        obs_params = obs_cfg.params if isinstance(obs_cfg.params, dict) else {}
        obs_dims = tuple(obs_func(env, **obs_params).shape)
